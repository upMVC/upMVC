# App\Modules\TestApi Enhanced API Documentation

## Overview
This is an enhanced API module for App\Modules\TestApi with auto-discovery capabilities.
...
```json
{
    "debug": true,
    "environment": "development",
    "module_path": "src/Modules/TestApi/",
    "auto_discovery": {
        "enabled": true,
...
const api = new EnhancedApp\Modules\TestApiModule();
...
$response = file_get_contents(BASE_URL . '/testapi/api');
$data = json_decode($response, true);
```

### cURL Example
```bash
curl -X GET "http://localhost/testapi/api" \
...
{strtoupper(App\Modules\TestApi)}_API_ENABLED=true
{strtoupper(App\Modules\TestApi)}_CACHE_TTL=3600
```
