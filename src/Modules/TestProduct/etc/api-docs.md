# App\Modules\TestProduct Enhanced API Documentation

## Overview
This is an enhanced API module for App\Modules\TestProduct with auto-discovery capabilities.
...
```json
{
    "debug": true,
    "environment": "development",
    "module_path": "src/Modules/TestProduct/",
    "auto_discovery": {
        "enabled": true,
...
const api = new EnhancedApp\Modules\TestProductModule();
...
$response = file_get_contents(BASE_URL . '/testproduct/api');
$data = json_decode($response, true);
```

### cURL Example
```bash
curl -X GET "http://localhost/testproduct/api" \
...
{strtoupper(App\Modules\TestProduct)}_API_ENABLED=true
{strtoupper(App\Modules\TestProduct)}_CACHE_TTL=3600
```
